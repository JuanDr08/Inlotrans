    ...
    try {
        ...
        for (const j of jornadas ?? []) {
            ...
            const base: Omit<LineaNomina, 'novedad' | 'detalleNovedad'> = {
                ...
                horaIn,
                horaOut,
                almuerzo: Math.round((r.minutos_almuerzo_descontados as number) / 60),
                horas,
                extraDiurna: Math.round((r.minutos_extras_ordinarias as number) / 60),
                extraNocturna: Math.round((r.minutos_extras_nocturnas as number) / 60),
                extraFestivaDiurna: Math.round((r.minutos_extras_dominical_festivo as number) / 60),
                extraFestivaNocturna: Math.round((r.minutos_extras_nocturna_dominical_festivo as number) / 60),
                recargoNocturno: Math.round((r.minutos_nocturnas as number) / 60),
                recargoFestivoNocturno: Math.round((r.minutos_domingos_festivos_nocturnos as number) / 60),
                recargoFestivo: Math.round(((r.minutos_festivos as number) + (r.minutos_domingos as number)) / 60),
                operacion: opNombre,
